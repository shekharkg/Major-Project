 ...
 */
public class GridAdapterBrands extends ArrayAdapter<ProductDataBrands> {
    ...
    public GridAdapterBrands(Context context, int resource) {
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        ViewHolder vh;
        if (convertView == null) {
            convertView = myLayoutInflater.inflate(R.layout.list_item_brands, parent, false);
            vh = new ViewHolder();
        ...
        vh.txtLineOne.setText(getItem(position).getBrand_names());
        ...
        Ion.with(vh.productImg).placeholder(R.drawable.product).error(R.drawable.product).load(getItem(position).getBrand_logo());

        /*vh.btnGo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View v) {
                Toast.makeText(getContext(), "Price : " +
                        getItem(position).getBrand_url(), Toast.LENGTH_SHORT).show();
            }
        });*/
    ...
    private double getRandomHeightRatio() {
        return (myRandom.nextDouble() / 2.0) + 0.3; // height will be 1.0 - 1.5 the width
    }
}
